...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ยิง Username และ Password ไปเช็กที่ API หลังบ้าน (เพื่อตรวจกับ AD)
      ...
      if (response.ok) {
        // ล็อกอินผ่าน AD สำเร็จ! เก็บข้อมูลและเด้งไปหน้า Dashboard
        localStorage.setItem('user', JSON.stringify(data.user));
        router.push('/dashboard');
      } else {
        // ถ้ารหัสผิด หรือ AD หาไม่เจอ แจ้งเตือน Error
        setError(data.error);
      }
    } catch (err) {
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12">
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 mt-8">
            ...
            {/* ส่วนแสดง Error หากล็อกอินไม่สำเร็จ */}
            {error && (
              <div className="p-3 bg-red-900/40 border border-red-500/50 text-red-200 text-sm font-medium rounded-lg animate-fade-in">
                ❌ {error}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3.5 px-4 font-semibold rounded-xl shadow-lg transition-all transform active:scale-[0.98] ${
                isLoading
